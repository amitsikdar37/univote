...
// ✅ Load ZK proof and public inputs\async function loadProofAndPublic() {
  try {
...
document.addEventListener('DOMContentLoaded', () => {
  const contractAddress = "YOUR_ZKP_VOTING_CONTRACT_ADDRESS";
  ...
  function getElectionIdFromURL() {
    return new URLSearchParams(window.location.search).get('electionId');
  }
    ...
    try {
      ...
      for (let i = 0; i < candidateCount.toNumber(); i++) {
        ...
        btn.onclick = () => {
          ...
          selectedCandidateIndex = parseInt(btn.dataset.index);
          submitVoteBtn.disabled = false;
          submitVoteBtn.textContent = `Vote for ${candidateName}`;
        };
  ...
  async function handleVoteProcess() {
    ...
    try {
      ({ proof, publicSignals } = await loadProofAndPublic());
    } catch (e) {
      statusMessageEl.textContent = "Error loading proof.";
      submitVoteBtn.disabled = false;
      return;
    }
    ...
    try {
      ...
      await tx.wait();
      statusMessageEl.textContent = "Vote successful!";
      submitVoteBtn.textContent = "Voted ✅";
    } catch (err) {
      statusMessageEl.textContent = "Vote failed.";
      submitVoteBtn.disabled = false;
      console.error(err);
    }
  }
  ...
  connectWalletBtn.onclick = connectAndLoad;
  submitVoteBtn.onclick = handleVoteProcess;

  const urlId = getElectionIdFromURL();
  if (urlId) {
    setElectionId(urlId);
    statusMessageEl.textContent = "Connect wallet to continue...";
  }
});
